...
<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8" session="false"%>
	<c:if test="${param.gv_viewType ne 'error' && param.gv_viewType ne 'login'}">
		...
		<c:if test="${fn:length(param.gdsNavigation) > 0}">
		<script type="text/javascript">
			$(document).ready(function() {
				localStorage.setItem('gdsNavigation', JSON.stringify(${param.gdsNavigation}));
			});
		</script>
		</c:if>
		<script src="/js/common_lib.js" type="text/javascript"></script>
		<script src="/js/actions/${param.gv_actionJs}?ver=${param.gv_timestamp}" type="text/javascript"></script>
		<!--end::Page Scripts -->
	</body>
	<!-- end::Body -->
</html>